...
const ShotPreparationSchema = z.object({
  grind_setting: z.number().int().min(1).max(50, 'Grind setting must be between 1 and 50').optional(),
  ...
  puck_thickness_mm: z.number().min(1).max(30, 'Puck thickness must be between 1 and 30mm').optional(),
...
const ShotExtractionSchema = z.object({
  ...
  extraction_time_seconds: z.number().min(1).max(300, 'Extraction time must be between 1 and 300 seconds').optional(),
...
const ShotEnvironmentSchema = z.object({
  ambient_temp_c: z.number().min(-10).max(50, 'Ambient temperature must be between -10°C and 50°C').optional(),
  humidity_percent: PercentageSchema.optional(),
  water_source: z.enum(['tap', 'filtered', 'bottled', 'reverse_osmosis', 'custom']).optional(),
  estimated_water_hardness_ppm: z.number().min(0).max(1000, 'Water hardness must be between 0 and 1000 ppm').optional(),
  ...
  shots_since_clean: z.number().int().min(0).max(100, 'Shots since clean must be between 0 and 100').optional(),
...
const ShotFeedbackSchema = z.object({
  ...
  flavor_notes: z.array(z.string().max(50, 'Flavor note must be less than 50 characters')).max(10, 'Maximum 10 flavor notes allowed').optional(),
  extraction_assessment: z.enum(['underextracted', 'optimal', 'overextracted', 'channeling', 'uneven']).optional(),
...
export const BulkShotIdsSchema = z.object({
  ids: z.array(UUIDSchema).min(1, 'At least one ID must be provided').max(50, 'Cannot process more than 50 items at once'),
});
