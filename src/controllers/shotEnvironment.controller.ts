...
export class ShotEnvironmentController {
    ...
    async save(request: Request, response: Response) {
        try {
            const { ambient_temp_c, humidity_percent, water_source, estimated_water_hardness_ppm, machine_warmup_minutes, shots_since_clean } = request.body;
            ...
            const environment = this.shotEnvironmentRepository.create({
                ...
                estimated_water_hardness_ppm: estimated_water_hardness_ppm ? parseInt(estimated_water_hardness_ppm) : null,
    ...
    async update(request: Request, response: Response) {
        try {
            ...
            // Update basic fields
            if (ambient_temp_c !== undefined) environment.ambient_temp_c = ambient_temp_c ? parseFloat(ambient_temp_c) : null;
            if (humidity_percent !== undefined) environment.humidity_percent = humidity_percent ? parseInt(humidity_percent) : null;
            if (water_source !== undefined) environment.water_source = water_source;
            if (estimated_water_hardness_ppm !== undefined) environment.estimated_water_hardness_ppm = estimated_water_hardness_ppm ? parseInt(estimated_water_hardness_ppm) : null;
            if (machine_warmup_minutes !== undefined) environment.machine_warmup_minutes = machine_warmup_minutes ? parseInt(machine_warmup_minutes) : null;
            if (shots_since_clean !== undefined) environment.shots_since_clean = shots_since_clean ? parseInt(shots_since_clean) : null;
